...
with tab2:
    st.header("🗂️ Batch Evaluation Mode")
    st.markdown("Run the full 5-stage doc-screening pipeline against all sample files in `sample_data/genuine` and `sample_data/forged`.")
    ...
    if run_batch:
        ...
        with st.spinner("Processing batch screening on all sample files..."):
            try:
                results = run_batch_evaluation(config=sidebar_config)
                metrics = compute_evaluation_metrics(results, config=sidebar_config)
            except Exception as e:
                st.error(f"Batch screening crashed: {e}")
                results = []

        if not results:
            st.error("No sample files found in sample_data/genuine or sample_data/forged.")
        else:
            # Metrics columns
            st.subheader("📊 Overall Batch Metrics")
            ...
            st.caption(f"Confusion Matrix: True Positives (Forged Flagged) = {cm['tp']} | False Positives (Gen Flagged) = {cm['fp']} | True Negatives (Gen Passed) = {cm['tn']} | False Negatives (Forged Passed) = {cm['fn']}")

            # Results Table
            st.subheader("📋 Results Table")
            df_res = pd.DataFrame(results)
            float_cols = ["validation_score", "tamper_score", "face_match_score", "risk_score"]
            ...
            # Score distributions
            st.subheader("📈 Score Distributions (Mean ± Std Dev)")
            dist_cols = st.columns(2)
            for i, (gt, dist) in enumerate(metrics["distributions"].items()):
                with dist_cols[i]:
                    st.write(f"**Ground Truth: {gt.upper()}**")
                    st.write(f"- Validation Score: {dist['validation']['mean']:.2f} ± {dist['validation']['std']:.2f}")
                    st.write(f"- Tamper Score: {dist['tamper']['mean']:.2f} ± {dist['tamper']['std']:.2f}")
                    st.write(f"- Face Match Score: {dist['face']['mean']:.2f} ± {dist['face']['std']:.2f}")
                    st.write(f"- Final Risk Score: {dist['risk']['mean']:.2f} ± {dist['risk']['std']:.2f}")
